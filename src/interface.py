...
class Interface:
    ...
    def main_program(self) -> None:
        ...
        dict_call_func = {"1": self.add_data, "2": self.print_data, "3": self.search_data, "4": self.edit_data}
        while True:
    ...
    def add_data(self) -> str:
        """Добавление данных"""
        new_data = self.preparation_data()
        return self.telephone_directory.add_entry(new_data)

    def print_data(self) -> str:
        """Вывод данных"""
        while True:
            page_number = input("Введите какую страницу справочника вывести: ")
            page_size = input("Введите количество записей которые нужно вывести: ")
    ...
    def search_data(self) -> str:
        ...
        while True:
            key_specification = input("По каким параметрам искать запись в справочнике?\n"
                                      "1. Имя, "
                                      "2. Фамилия, "
                                      "3. Отчество, "
                                      "4. Название организации, "
            ...
            keys = key_specification.split()
            if all(key in specification for key in keys):
                key_value_pairs = {}
                for key in keys:
                    value_specification = input(f"Введите {specification[key]} по которому искать: ")
    ...
    def edit_data(self) -> str:
        while True:
            index_entry = input("Введите номер записи, которую хотите отредактировать: ")
            index_entry = int(index_entry) - 1
    ...
    def preparation_data(self) -> dict:
        """Подготовка данных"""
        entry_directory = ("Имя", "Фамилия", "Отчество", "Название организации", "Телефон рабочий", "Телефон сотовый")
        new_data = {}
        for entry in entry_directory:
            while True:
                try:
                    value = input(f"Введите {entry}: ")
    ...
    def validate_data(self, entry: str, value: str) -> bool:
        ...
        string_value = ("Имя", "Фамилия", "Отчество", "Название организации")
        if entry in string_value:
